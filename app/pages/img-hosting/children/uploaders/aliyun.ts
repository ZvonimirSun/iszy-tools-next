...
export const type = 'aliyun'

export const name = '阿里云 OSS'
...
export const fields: FieldMeta[] = [
  ...
  { key: 'securityToken', label: 'Security Token', hint: 'SecurityToken（可选）', required: false, secret: true },
  { key: 'bucket', label: 'Bucket', hint: 'Bucket 名称', required: true },
  { key: 'region', label: 'Region', hint: '例如 oss-cn-beijing', required: true },
  { key: 'path', label: '存储路径', hint: '例如 img/', required: false },
  { key: 'customUrl', label: '自定义域名', hint: '例如 https://cdn.example.com', required: false },
  { key: 'options', label: '网址后缀', hint: '例如 ?x-oss-process=xxx', required: false },
]
...
function createClient(config: UploaderConfig): S3Client {
  ...
  return new S3Client({
    region: c.region,
    endpoint: `https://${c.region || ''}.aliyuncs.com`,
    credentials: {
      accessKeyId: c.accessKeyId || '',
      secretAccessKey: c.accessKeySecret || '',
      ...(c.securityToken ? { sessionToken: c.securityToken } : {}),
    },
  })
}
...
function buildUrl(config: UploaderConfig, fileKey: string): string {
  ...
  return `https://${c.bucket}.${c.region}.aliyuncs.com/${fullPath}${optionUrl}`
}
...
const aliyunUploader: Uploader = { type, name, fields, validate, upload, list, remove }
