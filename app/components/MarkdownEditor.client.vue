...
function getSelectedSource(placeholder: string) {
  const textarea = getSourceTextarea()
...
function wrapSource(prefix: string, suffix = prefix, placeholder = '文本') {
  const before = getSourceHistoryState()
  const markerRange = findSourceMarkerRange(before.value, before.selectionStart, before.selectionEnd, prefix, suffix)
  ...
  const { selected, hasSelection } = getSelectedSource(placeholder)
  const insert = `${prefix}${selected}${suffix}`
  const selectionStart = hasSelection ? insert.length : prefix.length
  const selectionEnd = hasSelection ? insert.length : prefix.length + selected.length
...
function toggleCodeBlock() {
  ...
  const { selected } = getSelectedSource('code')
...
<template>
  <div class="flex min-h-[calc(100vh-9rem)] w-full flex-col gap-4">
    <input
      ref="fileInput"
      type="file"
    ...
    <div class="min-h-0 flex-1 overflow-hidden rounded-md border border-default bg-default">
      <div
        v-if="mode === 'source'"
        ref="sourceEditor"
        class="flex h-full min-h-[32rem] flex-col"
      ...
      >
        ...
        <UTextarea
          ...
          :ui="{
            base: 'h-full min-h-[32rem] resize-none border-0 rounded-none focus-visible:ring-0',
          }"
      ...
      <div v-else-if="mode === 'split'" class="grid h-full min-h-[32rem] grid-cols-1 md:grid-cols-2">
        <div
          ref="sourceEditor"
          class="flex min-h-[32rem] flex-col border-b border-default md:border-r md:border-b-0"
          @click="updateSourceToolbarState"
          @keyup.capture="updateSourceToolbarState"
          @mouseup="updateSourceToolbarState"
          @select.capture="updateSourceToolbarState"
        ...
        <UEditor
          ...
          placeholder="输入 Markdown 内容..."
          class="h-full min-h-[32rem] overflow-auto"
          :ui="{
        ...
        >
          <template #default="{ editor }">
            <UEditorToolbar
              :editor="editor"
              :items="toolbarItems"
              class="sticky top-0 z-10 border-b border-default bg-default/95 px-3 py-2 backdrop-blur"
            />
          </template>
        </UEditor>
      </div>
