...
以下任务是近期迭代优先级，目标是在不增加服务端负担的前提下提升工具站质量。

### 1. 工具元信息增强

- 在 `shared/types/tool.ts` 中扩展工具元信息字段，例如 `requiresNetwork`、`aliases`、`keywords`。
- 在 `shared/data/tools.ts` 中优先为文件处理、网络请求、登录工具和重计算工具补齐这些字段。
- 首页、搜索和工具卡片只消费元信息，不在页面里重复硬编码工具说明。
- 工具元信息只描述通用入口、搜索、权限、核心功能是否必须联网等页面结构需要的内容；文件大小、格式限制、处理细节等工具内部约束应放在对应页面、组件或 composable 中。
- `requiresNetwork` 只描述核心功能是否离不开网络、接口、外部 API、远程资源或外链；不填或 `false` 表示核心功能可本地完成。登录同步、云保存、远程配置、URL 导入、远程依赖等可选增强能力，不进入工具级网络标记，由具体功能点自己判断。
- `requiresAuth` 继续保留为访问控制字段，不代表工具核心功能是否联网。
- 外链工具和内部工具后续可考虑区分类型，避免长期把 URL 塞进 `name`。

### 2. 首页搜索与入口体验

- 搜索范围从 `label/name/tags` 扩展到 `aliases/keywords/description`。
- 为常用中文工具补充别名，例如“二维码/QR”、“经纬度/坐标”、“哈希/hash”。
- 增加“最近新增/最近更新”或“推荐工具”这类轻量入口时，优先基于静态元信息或本地统计，不依赖服务端统计。
- 继续保持首页是工具入口，不做营销式落地页。

### 3. 文件与重计算工具前端化治理

- 盘点 PDF、图片、HASH、GIS、压缩/转换类工具，确认是否已在浏览器端完成处理。
- 对大文件处理在对应工具页面或组件内增加场景化提示、失败提示和必要的取消/重试能力。
- 计算量明显的逻辑优先迁移到 `children/*.worker.ts`，避免阻塞页面交互。
- 处理产物优先通过 Blob 下载或浏览器本地预览，不上传到应用服务器。

### 4. 本地数据导入导出

- 收藏、访问记录、工具设置、RunJS 片段、JSON 编辑器数据等优先提供本地导入/导出能力。
- 导出格式使用可读 JSON，并包含版本号，方便后续 store schema 迁移。
- 导入时要校验数据结构，避免损坏现有 IndexedDB 持久化状态。
- 账号云同步不是短期默认方向；需要同步时先从轻量设置开始。

### 5. PWA 与离线能力

- 优先让纯前端工具在静态资源已缓存时可离线打开。
- 工具元信息中使用 `requiresNetwork` 标记核心功能是否必须联网，用于后续在 UI 中提示本地可用/依赖网络。
- 不把需要后端认证、第三方 API 或实时网络的工具伪装成离线可用。
- PWA 缓存策略要克制，避免缓存用户隐私文件或过大的临时产物。
